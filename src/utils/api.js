...
/**
 * Retrieves all existing sources
 * @returns {Promise<[source]>}
 *  a promise that resolves to a possibly empty array of sources saved in the database.
 */
export async function listSources(signal) {
  const url = new URL(`${API_BASE_URL}/sources`);
  return await fetchJson(url, { headers, signal }, []);
}

/**
 * Retrieves all existing shows
 * @returns {Promise<[show]>}
 *  a promise that resolves to a possibly empty array of shows saved in the database.
 */
export async function listShows(signal) {
